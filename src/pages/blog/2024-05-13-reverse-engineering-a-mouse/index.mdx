...
import NoteContainer from "../../../components/blog/Note.astro";
...
![An AOC GM110 Gaming Mouse](gm110.jpg)
...
![Button binding page, where you can rebind mouse buttons to however you like](m1.png)  

![Macros page, which allows you to setup a sequence of key event that is triggered by a button (Trigger is setup by binding the mouse to a macro in the Mouse Binding Page)](m2.png)  

![Advanced page in the mouse software, with Fire Speed, "Pointer Precision" *(Bad Translation; Should be Pointer Speed)*, Scroll Speed and DPI Setup w/ 4 DPI to choose from](m3.png)

![LED Page in the mouse software, which allows you to configure the lighting of the mouse, with Slow, Fast, Static and Off available as option](m4.png)  
...
![KDE Mouse Settings Window, allowing you to rebind extra buttons](kde_rebind.png)
...
![Wireshark Installer on Windows, asking whether you want to install USBPCap](wireshark_usbpcap.png)
...
![Wireshark Home Screen, presenting a list of interfaces that can be captured. USBPCap appeared there.](wireshark_home.png)

And now I have access to all the USB data under the sun, ha-ha!

![Wireshark Window, with tons of USB Packets displayed](wireshark_spam.png)
...
![Wireshark Window, with usb packets filtered to usb address 1.2.1](wireshark_1.2.1.png)
...
![Wireshark Window, with a list of usb packets appearing after clicking the Apply button in the mouse software.](wireshark_on_apply.png)
...
![Button binding page, allowing you to not only rebind buttons, but also perform Shortcuts and perform Multimedia key input](mouse_binding_menus.png)
...
## Fire Speed
![Fire Speed Slider in the mouse software](fire_speed.png)
...
![Slow, Fast, Static and Off option presented in the LED page of the mouse software](rgb_options.png)
...
![Advanced page in the mouse software, allowing you to choose different DPI Profile](dpi_profiles.png)
...
![Advanced page in the mouse software, with a Pointer precision slider and a wheel speed slider](imposter.png)
...
![The Pointer precision slider in the mouse software and Windows mouse pointer speed settings side by side, showing that they match](windows_pointer_speed.png)
...
![A text editor showing my C prototype implementation](prototype1.png)
...
![My mouse light being turned off](rgb_off.png)  
...
<small>*God I need to thank my C++ teacher last semester.*</small>
...
![Qt Creator showing C++ code with repetitive functions and my newly written program side by side](qtwidget.png)
...
![The DPI Slider in the mouse software, with 12 slider overlay showing the possible value that the slider can be dragged onto](dpi_sliders.png)
...
![My mouse program, showing the DPI Page with 4 DPI profile to choose from, and a slider ranging between 200 - 3000](dpi_200_inc.png)
...
![The provided mouse software, there's a blue square on the top of each DPI selection that is actually a checkbox](dpi_checkboxes.png)
...
![Wireshark packet, highlighting the data '07 09 AD DC 0e 00 00 00'](wireshark_dpi_dump.png)
...
<NoteContainer title="Computer 101">
...
![The DPI Page in my program, now with Enabled checkboxes](dpi_enable_toggle.png)  
...
![Button binding page in the mouse software, allowing a button to be binded to an action called 'Fire'](fire_button_binding.png)

![The Fire Speed slider, which corresponds to the Fire action.](fire_speed.png)
...
<video controls src="/assets/dpi_sync_win.mp4" type="video/mp4"></video>
...
![Wireshark window, but I didn't apply a filter](wireshark_no_filter.png)

I clicked the DPI+ button again, and suddenly I see data sent from an address I have never seen before: **1.2.2**

![Wireshark window with bunch of USB Packet, with a highlighted packet from usb address 1.2.2](wireshark_1.2.2.png)
...
![My program's implementation for monitoring incoming input](input_handler_thread.png)
...
![Windows Desktop, hovering the program icon revealing a tooltips](a704f_tooltips.png)

It literally translates into "Instant A704F Gaming Mouse"  
I have never heard of that before, so I decided to look it up:

![Instant A704F Gaming Mouse SOC Datasheet](a704f.png)
...
![Instant A704 Driver Manual, which featured a different UI, with the option to use your mouse wheel to scroll or to control the sound volume](scroll_wheel.png)
...
![Wireshark USB Packet listing. First packet is the Mouse reported that the button is pressed. Second packet is the Mouse reported that the button is released](soft_action_dpi_click_nosoft.png)
![Wireshark USB Packet listing. First packet is the Mouse reported that the button is pressed. There's 2 more usb packets in between, and the last packet is the Mouse reported that the button is released](soft_action_dpi_click.png)   
...
![Button binding page from the mouse software, with a Shortcut submenu that offers actions like Copy/Paste](shortcut_menu.png)
...
### Macros!
If your brain has not exploded yet like me writing this blog post, you might have noticed that I never talked about the "Macros" feature after [listing it out in the beginning](#macros). That's because it is not a feature I use nor do I care about it, and it's also beyond the scope of this project.
...
![Linux Terminal, showing me run my prototype program](key_first_attempt.png)  
Nothing.  
...
Now back to my program. Since [earlier I've already made a monitor thread](#a-second-interface) to sync the active DPI, I can now just extend that to handle software key as well.

![My linux mouse program, with new codes inserted to the DPI monitoring code I wrote earlier](new_thread_code.png)
...
<video controls src="/assets/rgb_vfast.mp4" type="video/mp4"></video>  
...
![My linux mouse program's RGB Page, showing a new 'Very Fast' radio option that can be selected](rgb_tab_vfast.png)
...
![Sound volume got changed](vol.png)

This is inline with what I [observed earlier](#hidden-in-plain-sight) about there being an option to change your scroll wheel to control volume as well.
